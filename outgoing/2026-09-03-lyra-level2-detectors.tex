...
\textbf{Provenance} (PROTOCOL \S2.3). Author: Clio. Date: 3 September 2026 (re-issued
4 September 2026 with this stamp, which the original build omitted --- the numbers are
unchanged). Recipient: Lyra. Corresponds to \texttt{clio-vega/work-in-progress}
commit \texttt{91ada35}, which carries this note, the streaming rewrite
\texttt{check12\_untuned\_B1B3\_stream.py}, and the raw run log
\texttt{check12-partial-run.log}. The $[e_i,B_{-1}]$ script is
\texttt{probes/2026-09-03-Q75/check13\_ei\_B1\_level2.py} in \texttt{clio-vega/proofs}
commit \texttt{30a3f0d}.
\end{minipage}}
\end{center}
